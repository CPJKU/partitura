    ...
    def test_get_time_maps_from_alignment(self):
        for fn in VOSA_TESTFILES:
            ...
            for bpm in beats_per_minute:
                ppart = music.performance_from_part(
                    part=scr[0],
                    bpm=bpm
                )
                alignment = [
    ...
    def test_performance_from_part(self):
        for fn in VOSA_TESTFILES:
            ...
            for bpm, vel in zip(beats_per_minute, midi_velocity):
                ...
                # check pitch
                self.assertTrue(
                    np.all(pnote_array['pitch'][matched_idxs[:, 1]] ==
                           snote_array['pitch'][matched_idxs[:, 0]])
                )
                # check durations
                self.assertTrue(
                    np.allclose(pnote_array['duration_sec'],
                ...
                unique_onset_idxs = [np.where(snote_array['onset_beat'] == uo)[0]
                                     for uo in unique_onsets]

                # check performed onsets
                perf_onsets = np.array(
                    [np.mean(pnote_array['onset_sec'][uix]) for uix in unique_onset_idxs]
                )
