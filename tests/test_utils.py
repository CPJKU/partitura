...
class TestSegmentPpartByGap(unittest.TestCase):
    def make_ppart(self, note_times, note_durations):
            notes = []
    ...
    def test_no_gaps_returns_none(self):
        ppart = self.make_ppart([0, 1, 2, 3], [1, 1, 1, 1])
    ...
    def test_single_gap_splits_in_two(self):
        ppart = self.make_ppart([0, 1, 6, 11, 25, 26, 31, 35], [1, 1, 1, 1, 1, 1, 1, 1])
        segments, times = segment_ppart_by_gap(ppart, gap=10)
    ...
    def test_min_segment_duration_merges_short_segments(self):
        ppart = self.make_ppart([0, 1, 13, 14], [1, 1, 1, 1])
    ...
    def test_gap_at_end(self):
        ppart = self.make_ppart([0, 1, 2, 7, 11, 25], [1, 1, 1, 1, 1, 1])
        segments, times = segment_ppart_by_gap(ppart, gap=10)
