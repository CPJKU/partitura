...
from partitura.io.exportparangonada import (save_alignment_for_parangonada,
...
class TestIO(unittest.TestCase):
    ...
    def test_csv_import_export(self):
        with tempfile.TemporaryDirectory() as tmpdirname:
            save_alignment_for_parangonada(os.path.join(tmpdirname,"align.csv"), test_alignment)
            import_alignment = load_alignment_from_parangonada(os.path.join(tmpdirname,"align.csv"))
            equal = test_alignment == import_alignment
            self.assertTrue(equal)
    
    def test_tsv_import_export(self):
        with tempfile.TemporaryDirectory() as tmpdirname:
            save_alignment_for_ASAP(os.path.join(tmpdirname,"align.tsv"), test_ppart, test_alignment)
            import_alignment = load_alignment_from_ASAP(os.path.join(tmpdirname,"align.tsv"))
            equal = test_alignment == import_alignment
            self.assertTrue(equal)
