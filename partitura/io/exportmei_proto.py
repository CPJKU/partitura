...
# n0q = score.Note(id="n0q",step="G",octave=2,voice=2, staff=1)
# n1q =score.Note(id="n1q",step="G",octave=2,voice=2, staff=1)
# part.add(n0q,start=0,end=10)
# part.add(n1q,start=10,end=15)
        ...
        for s in notes_perStaff.keys():
            ...
            for voice,notes in notes_withinMeasure_perStaff_perVoice.items():
                ...
                for i in range(1,len(notes)):
                    ...
                    if chords[-1][0].start.t==n.start.t and chords[-1][0].duration==n.duration:
                        chords[-1].append(n)
                    else:
                        chords.append([n])
...
#                     n = chordNotes[0]
#                     dur_dots,splitNote = calc_dur_dots_splitNote(n)
#
#                     if isinstance(n,score.Note):
#                         if splitNote!=None:
#                             splitNote = score.Note(n.step,n.octave)
#
#
#                         openBeam=False
#                         parent = layer
...
#                             setAttributes(note,(xmlIdString,n.id),("pname",n.step.lower()),("oct",n.octave),("dur",dur_dots[0][0]),("dots",dur_dots[0][1]))
#
...
#                             openBeam = True
                    ...
                    elif isinstance(n,score.Rest):
                        if splitNote!=None:
                            splitNote = score.Rest()

                        rest = addChild(layer,"rest")

                        setAttributes(rest,(xmlIdString,n.id),("dur",dur_dots[0][0]),("dots",dur_dots[0][1]))
                    ...
                    if splitNote!=None:
                        splitNote.voice = n.voice
                        splitNote.start = m.end
                        splitNote.end = score.TimePoint(n.start.t+n.duration)
                        splitNote.id = n.id+"s"

                        if s in notes_nextMeasure_perStaff.keys():
                            notes_nextMeasure_perStaff[s].append(splitNote)
                        else:
                            notes_nextMeasure_perStaff[s]=[splitNote]
