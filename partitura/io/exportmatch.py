...
from partitura.io.importmatch import (MatchInfo,
                                      MatchMeta,
                                      MatchSnote,
                                      MatchNote,
                                      MatchSnoteNote,
...
from partitura.utils.music import (midi_pitch_to_pitch_spelling,
...
def matchfile_from_alignment(alignment, ppart, spart,
                             mpq=500000, ppq=480,
                             performer=None,
                             composer=None,
                             piece=None):
    ...
    header_lines = dict()
    header_lines['version'] = MatchInfo(Attribute='matchFileVersion',
                                        Value='5.0')
    if performer is not None:
        header_lines['performer'] = MatchInfo(Attribute='performer',
                                              Value=performer)
    ...
    header_lines['clock_units'] = MatchInfo(Attribute='midiClockUnits',
                                            Value=ppq)
    header_lines['clock_rate'] = MatchInfo(Attribute='midiClockRate',
                                           Value=mpq)
    ...
    measure_map = interp1d(measure_starts[:, 1], measure_starts[:, 0],
                           kind='previous', bounds_error=False,
                           fill_value=(measure_starts[:, 0].min(),
                                       measure_starts[:, 0].max()))
    ...
    for i, ts in enumerate(spart.iter_all(score.TimeSignature)):
        ...
        if i == 0:
            first_ts = MatchInfo(Attribute='timeSignature',
                                 Value='[{0}]'.format(value))
            header_lines['time_signature'] = first_ts

        ts_line = MatchMeta(Attribute='timeSignature',
                            Value=value,
    ...
    for i, ks in enumerate(spart.iter_all(score.KeySignature)):
        ...
        if i == 0:
            first_ks = MatchInfo(Attribute='keySignature',
                                 Value='[{0}]'.format(value))
            header_lines['key_signature'] = first_ks

        ks_line = MatchMeta(Attribute='keySignature',
                            Value=value,
    ...
    header_order = ['version', 'piece', 'composer', 'performer',
                    'clock_units', 'clock_rate', 'key_signature',
                    'time_signature']
    all_match_lines = []
    ...
    # Create match file
    matchfile = MatchFile.from_lines(lines=all_match_lines,
                                     name=ppart.part_name)

    return matchfile


def save_match(alignment, ppart, spart, out,
               mpq=500000, ppq=480,
               performer=None,
               composer=None,
               piece=None):
    ...
    """
    # Get matchfile
    matchfile = matchfile_from_alignment(alignment=alignment,
                                         ppart=ppart,
                                         spart=spart,
                                         mpq=mpq,
                                         ppq=ppq,
