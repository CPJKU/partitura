...
def parse_spline(spline):
    ...
    barline_indices = np.where(np.char.startswith(spline, "="))[0]
    # Find Chord indices, i.e. where spline cells contain " "
    chord_indices = np.where(np.char.find(spline, " ") != -1)[0]
    # Find Rest indices, i.e. where spline cells contain "r"
    rest_indices = np.where(np.char.find(spline, "r") != -1)[0]
    # All the rest are note indices
    non_note_indices = np.hstack((chord_indices, barline_indices, rest_indices))
    note_indices = ~np.isin(np.arange(len(spline)), non_note_indices, invert=True)
    # Get Notes
    notes = spline[note_indices]
...
if __name__ == "__main__":
    kern_path = "/home/manos/.struttura/ScriabinHumdrumDataset/op01/scriabin-op01.krn"
