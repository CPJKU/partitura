...
from partitura.musicanalysis.voice_separation import (VSNote,
                                                      VSBaseScore,
                                                      NoteStream,
                                                      Voice)
...
def _notearray_to_pianoroll(note_info, onset_only=False,
    ...
    if return_idxs:
        ...
        pr_idx = np.column_stack([pr_pitch, pr_onset, pr_offset]).astype(np.int)
...
def polyphonicslices_from_parts(parts, time_div=4, silent_voice_val=-1):
    """
    Get polyphonic slices from a list of `Part` objects. This method assumes
    that each `Part` is striclty monophonic.
    ...
    time_div : int, optional
        How many sub-division for each time unit (beats)
    silent_voice_val : float (default: -1)
        Value for indicating that a voice is silent.

    Returns
    -------
    polyphonic_slices: np.ndarray
        2D array with dimensions (num_timepoints, n_voices).
        The i-th
    time_points : np.ndarray
        Time positions (in beats) corresponding to each row in `polyphonic_slices`.

    TODO
    ----
    * Sort voices by range?
    * Are `Voice` objects necessary?
    """
    # number of voices
    n_voices = len(parts)
    # List to hold VSNote instances for each score note
    vs_notes = []
    # List to hold Voice instances
    voices = []

    # Iterate over the parts (vi will be used as index for the voice)
    for vi, part in enumerate(parts):
        # Get beat map for each element in the part
        bm = part.beat_map
        # List for holding the notes in the current voice
        v_notes = []
        # Iterate over all notes in the part
        for n in part.notes_tied:
            # Get onset and duration in beats for the current note
            onset, duration = bm([n.start.t, n.duration_tied])
            # Create a VSNote object from the current Note.
            vsnote = VSNote(pitch=n.midi_pitch,
                            onset=onset,
                            duration=duration,
                            voice=vi, # assign the voice to the part
                            note_id=n.id)
            v_notes.append(vsnote)

        # Extend list of notes
        vs_notes += v_notes

        # Create a `Voice` object from the notes in the part 
        stream = NoteStream(notes=v_notes,
                            voice=vi)
        voice = Voice(stream_or_streams=stream,
                      voice=vi)
        voices.append(voice)
        
    # Create a VSBaseScore instance and set the voices
    vsscore = VSBaseScore(vs_notes)
    vsscore.voices = voices

    return _vsscore_to_polyphonicslices(vsscore=vsscore,
                                        time_div=time_div,
                                        silent_voice_val=silent_voice_val)

def polyphonicslices_from_notearray(note_array, time_div=4, silent_voice_val=-1):
    """
    Get polyphonic slices from a `note_array`. This method assumes that each voice is
    strictly monophonic.
    ...
    * Sort voices by range?
    ...
    n_voices = len(np.unique(note_array['voice']))
    v_idxs = dict([(vi, i) for i, vi in enumerate(np.unique(note_array['voice']))])

    vs_notes = []
    for i, n in enumerate(note_array):
        n_id = 'n{0}'.format(i) if (n['id'] == 'None' or n['id'] is None) else n['id']
        
        vsnote = VSNote(pitch=n['pitch'],
                        onset=n['onset'],
                        duration=n['duration'],
                        voice=v_idxs[n['voice']],
                        note_id=n_id)

        vs_notes.append(vsnote)

    vsscore = VSBaseScore(vs_notes)

    return _vsscore_to_polyphonicslices(vscore=vscore,
                                        time_div=time_div,
                                        silent_voice_val=silent_voice_val)
    

def _vsscore_to_polyphonicslices(vsscore, time_div, silent_voice_val):
    """
    Compute polyphonic slices from a VSBaseScore instance

    Parameters
    ----------
    vsscore : partitura.musicanalysis.voice_separation.VSBaseScore
       Instance of `VSBaseScore` with voice information for each
       note.
    time_div : int, optional
        How many sub-division for each time unit (beats)
    silent_voice_val : float (default: -1)
    ...
    Notes
    -----
    * This method assumes that each note in the input  `vsscore`
    has voice information and that each voice is strictly monophonic.
    """
    first_onset = min([n.onset for n in vsscore.notes])
    last_offset = max([n.offset for n in vsscore.notes])
    n_voices = max([n.voice for n in vsscore.notes]) + 1

    # Create `time_points` array
    step = 1 / time_div
    time_points = np.arange(first_onset, last_offset + step, step=step)

    # initialize array to hold polyphonic_slices (as if all voices were silent
    # all the time)
    polyphonic_slices = np.ones((len(time_points), n_voices)) * silent_voice_val
    
    # Assign values to the rows of the `polyphonic_slices` array
    for i, tp in enumerate(time_points):
        # Get notes sounding at current time point
        sounding_notes = vsscore.sounding_notes(tp)
        for n in sounding_notes:
            # Assign the pitch of the note to the voice
            polyphonic_slices[i][n.voice] = n.pitch

    return polyphonic_slices, time_points
