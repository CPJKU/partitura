...
key_prof_min_kp = np.array(
    [0.712, 0.048, 0.474, 0.618, 0.049, 0.460,
...
def _similarity_with_pitch_profile(note_array, key_profiles=KRUMHANSL_KESSLER,
                                   similarity_func=corr,
    ...
    pitch_distribution = np.array([note_array[duration_unit][
        np.where(pitch_classes == pc)[0]].sum()
                                   for pc in range(12)])
